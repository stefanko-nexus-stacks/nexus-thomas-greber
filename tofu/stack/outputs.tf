...
output "ssh_command" {
  ...
  value       = "cloudflared access ssh --hostname ssh-thomas-greber.nona.company"
}
...
output "ssh_config" {
  ...
  value       = <<-EOT
    Host nexus
      HostName ssh-thomas-greber.nona.company
      User root
...
output "service_urls" {
  ...
  value = {
    ...
    key => "https://${service.subdomain}-thomas-greber.nona.company"
  }
}
...
output "secrets" {
  ...
  value = {
    ...
    redpanda_schema_registry_public_url = "http://redpanda-schema-registry-thomas-greber.nona.company:18081"
